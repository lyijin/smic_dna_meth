...
tsv_files = Path.cwd().glob('???.subsamp.bona_fide.meth_density.tsv')
for t in tsv_files:
    temp = pd.read_table(t, usecols=[0, 11, 15, 19])
    temp['temperature'] = t.name[:2]
    temp['replicate'] = t.name[2]
    df.append(temp)

df = pd.concat(df, axis=0, ignore_index=True)
df.columns = [x.replace('within_meth_', '') for x in df.columns]

# exclude SINEs, rRNA and snRNA repeat regions from plotting because there's
# too few of them. also exclude sums and nots, because this plot focuses on
# individual repeat element types
excluded_repeats = ['SINE', 'rRNA', 'snRNA', 'sum_repeats', 'not_repeats']
df = df[~df['repeat_family'].isin(excluded_repeats)]
df = df.reset_index(drop=True)

# harness the power of "groupby"s to calculate stats of specific groups
df_means = df.groupby(['repeat_family', 'temperature']).mean()
df_sems = df.drop(['replicate'], axis=1).groupby(['repeat_family', 'temperature']).sem()
...
yloc = [0.2, 0.5, 0.8, 1.2, 1.5, 1.8, 2.2, 2.5, 2.8]
...
plt.yticks(yloc, ['26', '29', '32'] * 3)
plt.text(-3, 0.5, 'CpG', verticalalignment='center', rotation=90)
plt.text(-3, 1.5, 'CHG', verticalalignment='center', rotation=90)
plt.text(-3, 2.5, 'CHH', verticalalignment='center', rotation=90)
