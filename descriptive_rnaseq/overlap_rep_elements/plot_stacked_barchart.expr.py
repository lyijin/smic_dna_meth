...
# exclude SINEs, snRNA and RC repeat regions from plotting because there's
...
excluded_repeats = ['SINE', 'snRNA', 'RC', 'not_repeats']
df = df[~df['repeat_family'].isin(excluded_repeats)]
df = df.reset_index(drop=True)

# naming hack: in order to place "rRNA" in between "Low complexity" and 
#              "Satellite" for aesthetic reasons, rRNA --> RRNA.
#              sorting with custom function will be possible in the unreleased
#              pandas v1.0.0
# this will be reverted during post-plotting image processing
df['repeat_family'] = df['repeat_family'].map(lambda x: x.replace('rRNA', 'RRNA'))
